...
//	public static final String TEMPLATE_FILENAME = "template_udo.pdf";
	...
	public InputStream getTemplateResource() {
		InputStream resourceAsStream = this.getClass().getClassLoader().getResourceAsStream(KugUtils.TEMPLATE_ODB_FILENAME);
	...
	public void createMapping(String filePrefix) throws FileNotFoundException, IOException {
		...
		try (FileOutputStream fileOutputStream = new FileOutputStream(new File(filePrefix + ".pdf"));
			...
			csvString.append(String.format("%s;%s;%s;%s;%s%n", "ownId", "getFullyQualifiedName()",
					"getPartialName()",
	...
	public void dumpFields() throws FileNotFoundException, IOException {
		byte[] template = IOUtils.toByteArray(getTemplateResource());

		PDDocument doc = PDDocument.load(new ByteArrayInputStream(template));
		PDDocumentCatalog docCatalog = doc.getDocumentCatalog();
		PDAcroForm acroForm = docCatalog.getAcroForm();
		Iterator<PDField> fieldIterator = acroForm.getFieldIterator();
		while (fieldIterator.hasNext()) {
			PDField next = fieldIterator.next();
			System.out.printf("%-70s %-70s %-30s %n", next.getPartialName(), next.getAlternateFieldName(),
					next.getFieldType());
	...
	// Stamm-Nr_Kug[0]
	public void dumpFieldsToCsv(String csvFileName) throws FileNotFoundException, IOException {
		...
		PDAcroForm acroForm = docCatalog.getAcroForm();
		Iterator<PDField> fieldIterator = acroForm.getFieldIterator();
		retVal.append(String.format("%s;%s;%s;%s%n", "getFullyQualifiedName()", "getPartialName()",
				"getAlternateFieldName()", "getFieldType()"));
		while (fieldIterator.hasNext()) {
			PDField next = fieldIterator.next();
			retVal.append(String.format("%s;%s;%s;%s%n", next.getFullyQualifiedName(), next.getPartialName(),
					(next.getAlternateFieldName() != null ? next.getAlternateFieldName().replace("\r", " ") : ""),
					next.getFieldType()));
		}
