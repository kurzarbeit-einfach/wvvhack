...
public class KugInstantiator {
	...
	public File saveInstance(AnzeigeArbeitsausfall anzeige, String fileName) {
		...
		try (FileOutputStream fileOutputStream = new FileOutputStream(retVal);
				ByteArrayOutputStream createInstance = new KugInstantiatorDelegator().createInstance(anzeige);) {
	...
	public ByteArrayOutputStream createInstance(AnzeigeArbeitsausfall anzeige) throws IOException {
		return new KugInstantiatorDelegator().createInstance(anzeige);
	}

	private static class KugInstantiatorDelegator {

		private PDDocumentCatalog kugDocumentCatalog;
		private PDAcroForm kugAcroForm;

		ByteArrayOutputStream createInstance(AnzeigeArbeitsausfall anzeige) throws IOException {
			try (ByteArrayOutputStream retVal = new ByteArrayOutputStream();
					final PDDocument kugDocument = PDDocument.load(KugUtils.INSTANCE.getTemplateResource())) {
				...
				this.setTextValue("vereinbart_am",
						anzeige.getKurzarbeitVereinbartDurchAenderungskuendigungVereinbartAm());
				this.setTextValue("vereinbart_wirkung_am",
						anzeige.getKurzarbeitVereinbartDurchAenderungskuendigungWirksamZu());
				...
				this.setTextValue("anzahl_arbeitnehmer_betroffen_kurzarbeit",
						anzeige.getAnzahlArbeitnehmerMitEntgeltAusfall());
